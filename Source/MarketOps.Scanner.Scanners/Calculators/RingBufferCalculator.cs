...
internal static class RingBufferCalculator<T>
{
    ...
    public static T[] Calculate(T[] data, int bufferLength, int resultLength, Func<T[], T> operation)
    {
    ...
    private static bool CanCalculate(T[] data, int bufferLength, int resultLength) =>
    ...
    private static T[] CalculateResult(RingBuffer<T> buffer, T[] data, int resultLength, Func<T[], T> operation)
    {
